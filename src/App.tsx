...
function Shell({ children }: { children: ReactNode }) {
  ...
  const nav = [
    ...
    { href: '/what-if', label: 'What-If scenario', icon: SlidersHorizontal },
    { href: '/forecast', label: 'Forecast studio', icon: CloudSun },
...
function Overview() {
  ...
  return <div className="mx-auto max-w-[1450px] fade-up">
    <div className="mb-8 flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><div className="eyebrow text-[hsl(var(--primary))]">Monday · 16 June 2025 · {orgName}</div><h1 className="mt-3 max-w-xl text-3xl font-extrabold tracking-[-.04em] md:text-4xl">Good morning, {firstName}.<br /><span className="text-[hsl(var(--primary))]">{roleSubtitles[currentRole] || 'Here is what needs your attention.'}</span></h1></div><div className="flex flex-wrap items-center gap-2 self-start lg:self-end"><Link href="/what-if" data-testid="link-overview-whatif" className="flex items-center gap-2 rounded-xl border bg-[hsl(var(--card))] px-3.5 py-3 text-xs font-bold hover:bg-[hsl(var(--muted))] transition"><SlidersHorizontal size={15} /> What-If Studio</Link><Link href="/forecast" data-testid="link-overview-forecast" className="flex items-center gap-2 rounded-xl border bg-[hsl(var(--card))] px-3.5 py-3 text-xs font-bold hover:bg-[hsl(var(--muted))] transition"><CloudSun size={15} /> Forecast Studio</Link><Link href="/simulation" data-testid="link-overview-simulation" className="group flex items-center gap-3 rounded-xl bg-[hsl(var(--primary))] px-4 py-3 text-sm font-bold text-[hsl(var(--primary-foreground))] shadow-lg shadow-emerald-900/10">Open live demo <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" /></Link></div></div>
      ...
      <Route>
        <Shell>
          <Switch>
            ...
            <Route path="/forecast" component={ForecastStudioPage} />
